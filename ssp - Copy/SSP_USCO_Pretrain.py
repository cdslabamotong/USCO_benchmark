...
def main():
    ...
    parser.add_argument(
        '--preTrainPath', default="/pre_train/col/col_uniform_6400_160_6400/", 
    ...
    print("==============")
    print(non_zero_count/len(w))
    print("==============")
    
    index_weight ={}
    for index, weight in zip(featureIndexes,w):
        index_weight[index]=weight
        
    sorted_index_weight = sorted(index_weight.items(), key=lambda kv: kv[1],reverse=True)
    featureIndexes = []
    w=[]
    for item in sorted_index_weight:
        if len(featureIndexes)<int(1*featureNum):
            featureIndexes.append(item[0])
            w.append(item[1])
